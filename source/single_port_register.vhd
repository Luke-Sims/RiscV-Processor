...
entity single_port_register is
...
architecture rtl of single_port_register is
	...
	-- default value.
	signal reg : memory_t := init_register;
begin
	process(clk,reset)
	begin
	    if reset then
            reg(31 downto 0) <= init_register;
		elsif(rising_edge(clk)) then
			if (we = '1') then
			    reg(to_integer(unsigned(rw))) <= data;
				reg(0) <= std_logic_vector(to_unsigned(0, N));
			end if;
		end if;
	end process;
	busA <= reg(to_integer(unsigned(ra)));
	busB <= reg(to_integer(unsigned(rb)));
end rtl;
